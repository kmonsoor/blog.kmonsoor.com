...
## Getting started
 - make sure all the resources(links, images, CSS & JS files) you mentioned commonly in your pages are valid
 - check browser's `console` in `Developers tools` for any errors e.g. unavailable urls, faulty html/css etc.
...
It's that `Github.com`(or where your repo is hosted on) is stronger domain than yours, so Google "sees" that your site has contents (even though it's yours) that's also on Github, then there's a high chance that it'll mark your contents as "duplicate". And, duplicate contents gets heavy hammers from Google's SEO point-of-view. Searching any writeup from yours, even it's very unique in the cyber "wasteland", Google search will show both your site and the repo as well, possibly links from your site will be on the lower side.
...
For example, Github.com's [robot.txt](https://github.com/robots.txt) allows Google (or any search engine for that matter) to index only the `master` branch of any open-source repo. So, if your repo don't have any branch named `master`, it won't be indexed. That's it. Rename your "master" as "live", "main", "production" or whatever you feel like.
 ...
 2. Tell Github about new move
  ...
  ![change repo-settings on Github](http://i.imgur.com/wjf6zwu.png)
...
But try to keep it less 60 characters or search-engines will truncate it. Use each characters wisely.

It may look like this in your `base.html`.
The `else` clause below is to ensure that non-article pages also get a title, even it's just your "tagline" defined in `pelicanconf.py`.
...
If you have `www`, `http` and `https` versions of your site, tell Google here which one is preferred. It's only applicable to your domain-root. Then, all the search-results from your site will show that preffered version of your site.

![setting preference for www or non-www version](http://i.imgur.com/51JY1oe.png)

You have to add and do it both for `http` and `https` version of your site, if you have both.
...
**or**, you may want to do it through site's `.htaccess` file, by adding the following.
...
#### Check index-status
 Check for Google's index status of your site on the `search-console`.
...
Though, OpenGraph originated from facebook Inc., these tags are now widely used by other social engines, even by Google+. In absence of Twitter tags, Twitter uses these `og` tags. Try to include both `og:` and `twitter:` tags. Proper data in these tags makes your article cleanly-sharable in these social sites.

The below snippet that [I use myself](https://github.com/kmonsoor/blog.kmonsoor.com/blob/pelican-how-to-make-seo-friendly/plumage/templates/base.html) can be your starting point.
...
#### Compress everything
  Speed don't impacts SEO directly, but Google punish slow-site especially when search is made on a mobile device.

   So, make sure all static files are compressed. If not, compress theme's JS, CSS files yourself.
   Or, better to use `[gzip_cache](https://github.com/getpelican/pelican-plugins/tree/master/gzip_cache)` and `[yuicompressor](https://github.com/getpelican/pelican-plugins/tree/master/yuicompressor)` plugins for Pelican.

#### CDN

 * Use CDN-ed versions of common libraries(e.g. jQuery, Bootstrap etc.) rather than hosting your own copy, unless your theme actively modified it.
...
### Engage commenting
 ...
 Being a static site, integrating commenting-system looks a little far-fetched. However, blogs without proper commenting system feels kinda lame sometimes. Of course, YMMV.
...
### Headers
 * Don't use multiple 1st-level headers `H1` style. The main title link probably already have used it once.
  So, avoid it anymore, meaning avoid underlined-style(`=======`) or hash-style(single '#') headers in your markdown files.

  However, 2nd-level `H2` tags can(and should) be used multiple time. In case of markdown files, that's `---------` underlines, or line starting with double-hash('##').

I hope to grow this post, as i gain more insights. Thanks for reading down so far. Adios !
